...
const VulnerableCoursePlatform = () => {
  ...
  const internalCoupons = {
    "3atwa": {
      discount: 40,
      uses: -1,
      description: "Unlimited uses - $40 off per use",
    },
  ...
  const courses = [
    ...
    },
    {
      ...
      rating: 4.9,
      description:
        "Design and implement secure cloud infrastructure. Focus on AWS, Azure, and GCP security best practices.",
      topics: [
        "Cloud Architecture",
        "IAM",
        "Network Security",
    ...
    },
    {
      ...
      rating: 4.6,
      description:
        "Comprehensive mobile security for iOS and Android. Learn to identify and fix mobile app vulnerabilities.",
      topics: [
        "Mobile OWASP",
        "iOS Security",
        "Android Security",
  ...
  const addToCart = (course) => {
    if (!cart.find((c) => c.id === course.id)) {
      ...
      pendo.track("course_added_to_cart", {
        courseId: course.id,
  ...
  const logAttempt = (msg) => {
    setAttemptLog((prev) =>
      [{ ts: new Date().toISOString(), msg }, ...prev].slice(0, 80),
    );
  };
  ...
  const handleSsrfRequest = async () => {
    ...
    if (
      ssrfUrl.includes("localhost") ||
      ssrfUrl.includes("127.0.0.1") ||
      ssrfUrl.includes("internal") ||
      ssrfUrl.includes("::1") ||
      ssrfUrl.includes("[::1]") ||
      ssrfUrl.includes("127...1")
    ) {
      if (ssrfUrl.includes("/api/coupons")) {
        ...
        setSsrfResponse(
          "ERROR: Endpoint not found on internal server. Try /api/internal/coupons or /api/availability",
        );
        showMessage(
          "Internal endpoint not found. Explore available endpoints.",
          "error",
        );
      }
      pendo.track("availability_check_completed", {
        isInternalUrl: true,
        endpointPath: ssrfUrl.includes("/api/coupons")
          ? "/api/coupons"
          : ssrfUrl.includes("/api/availability")
            ? "/api/availability"
            : ssrfUrl.includes("/api/health")
              ? "/api/health"
              : "unknown",
        responseType: ssrfUrl.includes("/api/coupons")
          ? "coupons"
          : ssrfUrl.includes("/api/availability")
            ? "availability"
            : ssrfUrl.includes("/api/health")
    ...
    try {
      ...
      showMessage(
        "Failed to fetch the URL. Make sure /api/availability is running on the lab server.",
        "error",
      );
      pendo.track("availability_check_completed", {
        isInternalUrl: false,
        endpointPath: ssrfUrl,
        responseType: "remote_error",
        fetchSuccess: false,
      });
    }
  };
  ...
  const applyCoupon = async () => {
    ...
    logAttempt(
      `📤 Request sent: ${couponCode} (concurrent: ${concurrentRequests + 1})`,
    );
    ...
    try {
      ...
      if (response.ok && result.success) {
        ...
        setAppliedDiscounts((prev) => {
          ...
          logAttempt(
            `✅ Coupon applied: ${couponCode} - $${result.discount} off (Remaining: ${result.remaining})`,
          );
          showMessage(
            `Coupon applied! $${result.discount} off added`,
            "success",
          );
          return updated;
        });
        ...
        pendo.track("coupon_applied", {
          couponCode: couponCode,
          discountAmount: result.discount,
          remainingUses: result.remaining,
          appliedDiscountCount: appliedDiscounts.length + 1,
          cartSubtotal: cart.reduce((sum, c) => sum + c.price, 0),
          newCartTotal: Math.max(
            0,
            cart.reduce((sum, c) => sum + c.price, 0) -
              (getTotalDiscount() + result.discount),
          ),
        });
      } else {
        logAttempt(`❌ Failed: ${result.error || "Unknown error"}`);
        showMessage(result.error || "Failed to apply coupon", "error");
        pendo.track("coupon_application_failed", {
          couponCode: couponCode,
          errorMessage: (result.error || "Unknown error").substring(0, 100),
          failureType: "server_rejection",
        });
      }
    } catch (error) {
  ...
  const completePurchase = () => {
    ...
    if (used3atwa && multipleApplications && total === 0) {
      ...
      pendo.track("purchase_completed_free", {
        courseCount: cart.length,
        originalSubtotal: cart.reduce((sum, c) => sum + c.price, 0),
        totalDiscount: getTotalDiscount(),
        appliedCouponCount: appliedDiscounts.length,
        couponCodesUsed: [...new Set(appliedDiscounts.map((d) => d.code))].join(
          ", ",
        ),
      });
    } else {
  ...
  const processPayment = () => {
    ...
    const total = calculateTotal();
    showMessage(
      `Payment processed! Total charged: $${total.toFixed(2)}`,
      "success",
    );

    pendo.track("payment_processed", {
      totalCharged: total,
      courseCount: cart.length,
      cartSubtotal: cart.reduce((sum, c) => sum + c.price, 0),
      totalDiscountApplied: getTotalDiscount(),
    ...
    setTimeout(() => {
      ...
      setCreditCard("");
      showMessage(
        "Purchase complete! But you didn't exploit both vulnerabilities correctly.",
        "info",
      );
    }, 800);
  };
    ...
    return (
      ...
      >
        ...
        >
          ...
          />
          <h1 style={{ fontSize: 40, fontWeight: 800, color: "#222" }}>
            🎉 Congratulations! 🎉
          </h1>
          ...
          >
            <p style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
              ...
              />
              <strong>SSRF Exploitation:</strong>&nbsp;Used Server-Side Request
              Forgery to access internal API at localhost/127.0.0.1 and retrieve
              coupon codes from /api/coupons
            </p>
            <p
              style={{
                display: "flex",
            ...
            >
              <CheckCircle
                style={{ width: 20, height: 20, color: "#16a34a" }}
              />
              <strong>Race Condition Exploitation:</strong>&nbsp;Exploited
              timing vulnerability by rapidly clicking Apply button to stack the
              $40 coupon multiple times, reaching $0 total
            </p>
          </div>
  ...
  if (showPaymentModal) {
    return (
      ...
      >
        ...
        >
          <h2 style={{ fontSize: 24, fontWeight: 800, color: "#222" }}>
            Payment Required
          </h2>
          <p style={{ color: "#555", marginTop: 8 }}>
            Your total is{" "}
            <strong style={{ color: PALETTE.primary, fontSize: 20 }}>
              ${calculateTotal().toFixed(2)}
            </strong>
            . Please enter your credit card to complete the purchase.
          </p>
          ...
          <div style={{ marginTop: 14 }}>
            ...
            <input
              ...
              value={creditCard}
              onChange={(e) =>
                setCreditCard(e.target.value.replace(/\D/g, "").slice(0, 16))
              }
          ...
          >
            💡 Hint: The goal is to get the total to $0 without needing to pay!
          </p>
        </div>
      </div>
    );
    ...
    <div style={{ minHeight: "100vh", backgroundColor: PALETTE.white }}>
      ...
      <header
        style={{
          backgroundColor: PALETTE.primary,
          color: PALETTE.white,
          boxShadow: "0 4px 12px rgba(84,121,247,0.18)",
        }}
      >
        <div
          style={{
          ...
          <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
            ...
            <div>
              <h1 style={{ margin: 0, fontSize: 20, fontWeight: 800 }}>
                SecureCourses Academy
              </h1>
              <p style={{ margin: 0, fontSize: 12, color: "#e6efff" }}>
                Learn Security, Build Your Future
              </p>
            </div>
          </div>
          ...
          >
            <div>
              ...
              >
                ...
                >
                  <h2
                    style={{ margin: "0 0 8px", fontSize: 18, fontWeight: 800 }}
                  >
                    About This Course
                  </h2>
            ...
            >
              ...
              >
                ...
                >
                  <ExternalLink
                    style={{ width: 18, height: 18, color: PALETTE.primary }}
                  />{" "}
                  Check Course Availability
                </h3>
                <p style={{ color: "#555", marginTop: 8, fontSize: 13 }}>
                  Enter a URL to verify course availability in our system (lab
                  only)
                </p>
                <input
                  type="text"
          ...
          >
            <div>
              ...
              >
                ...
                >
                  <ShoppingCart style={{ width: 18, height: 18 }} /> Shopping
                  Cart ({cart.length})
                </h2>
                ...
                {cart.length === 0 ? (
                  <div style={{ textAlign: "center", padding: 40 }}>
                    <ShoppingCart
                      style={{ width: 64, height: 64, color: "#c7d2fe" }}
                    />
                  ...
                  <div style={{ marginTop: 12 }}>
                    ...
                    {appliedDiscounts.length > 0 && (
                      <div
                        style={{
                          marginTop: 10,
                          borderTop: "1px dashed #e6f0ff",
                          paddingTop: 10,
                        }}
                      >
                        <p style={{ color: "#059669", fontWeight: 800 }}>
                          Applied Discounts:
                        </p>
        ...
        <div style={{ maxWidth: 1200, margin: "20px auto", padding: 16 }}>
          ...
          >
            {courses.map((course) => (
              ...
              >
                <div
                  style={{ height: 84, backgroundColor: PALETTE.secondary }}
                ></div>
